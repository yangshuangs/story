<?php
	...
	*/
	class WebBase extends DBM {
	     ...
	     */ 
		function login_session($code, $rawData, $signature){
			$this->code 		= $code;
			...
			if (strlen($this->code) == 0){
				...
				if (isset($response['errcode'])){
					...
					// 校验用户信息
					if ($this->check_signature($this->session_key, $this->rawData, $this->signature)){
						$uinfo = json_decode($this->rawData, true);
						// session存在
		...
		// 		if ($this->update("UPDATE t_user SET nickname='".$nickname."',avatar_url='".$avata_url."',gender='".$gender."',city='".$city."' WHERE openid = '".$this->openid."'")){
		// 			return true;
		// 		}
		// 	}
		// 	return false;
		...
		function insert_user_info(array $uinfo){
			if (isset($uinfo['nickName'])
				...
				$city = $uinfo['city'];
				$avata_url = $uinfo['avatarUrl'];
				if ($this->insert("insert into t_user(openid, nickname, avatar_url, gender, city) values ('".$this->openid."','".$nickname."','".$avata_url."',".$gender.",'".$city."')")){
					return true;
				}
			}
			return false;
	     ...
	     * @param string signature 数据签名
	     */ 
	    function check_signature($session_key, $rawData, $signature) {
